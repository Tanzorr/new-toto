...
export class UserService {
  ...
  addUser(user: UserCreateData) {
    return this.http.post<CreateUserResponse>(this.baseUrl + 'users', user, { headers: this.headers });
  }
  ...
  updateUser(value: User): Observable<User> {
    ...
    return this.http.put<User>(this.baseUrl + 'users/' + value.id, value,  { headers: this.headers });
  }
}
